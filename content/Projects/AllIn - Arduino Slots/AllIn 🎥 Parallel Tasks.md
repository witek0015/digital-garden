...
Tags: [AllIn]
---
Arduino ususally does things one at a time
...
This code allows for doing things separately. It does so by calling functions tat check the state, and then a function that updates the state. The function *millis()* does the heavy lifting here.
Additionally, one user recommended using switch statements instead of ifelse:
